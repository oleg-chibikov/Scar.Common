...
public class AssemblyInfoProvider : IAssemblyInfoProvider
{
    ...
    {
        _ = specialPathsProvider ?? throw new ArgumentNullException(nameof(specialPathsProvider));
        _ = entryAssemblyProvider ?? throw new ArgumentNullException(nameof(entryAssemblyProvider));

        var entryAssembly = entryAssemblyProvider.ProvideEntryAssembly();
